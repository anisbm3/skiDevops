...
public class SubscriptionServicesImpl implements ISubscriptionServices{
    ...
    public void showMonthlyRecurringRevenue() {
        Float monthlyRevenue = getRevenue(TypeSubscription.MONTHLY);
        Float semiAnnualRevenue = getRevenue(TypeSubscription.SEMESTRIEL) / 6;
        Float annualRevenue = getRevenue(TypeSubscription.ANNUAL) / 12;

        // Handle potential null values
        Float totalRevenue = (monthlyRevenue != null ? monthlyRevenue : 0) +
                (semiAnnualRevenue != null ? semiAnnualRevenue : 0) +
                (annualRevenue != null ? annualRevenue : 0);

        log.info("Monthly Revenue = " + totalRevenue);
    }

    private Float getRevenue(TypeSubscription typeSub) {
        Float revenue = subscriptionRepository.recurringRevenueByTypeSubEquals(typeSub);
        return revenue != null ? revenue : 0.0f; // Return 0.0f if revenue is null
    }
}
